...
class TestSignUpView(TestCase):
    ...
    def test_post_with_invalid_email(self):
        ...
        data['email'] = 'test'
        res = self.client.post('/account/signup/', data)
        self.assertEqual(res.context['form'].errors['email'], ['Enter a valid email address.'])
        self.assertEqual(res.status_code, 200)
    ...
    def test_post_with_already_exists_phone(self):
        ...
        res = self.client.post('/account/signup/', data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.context['form'].errors['phone'], ['This field is required.'])
        self.assertEqual(User.objects.count(), 1)
    ...
    def test_post_with_already_exists_wrong_password(self):
        ...
        res = self.client.post('/account/signup/', data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.context['form'].errors['password'], ['Passwords do not match.'])
...
class TestLoginView(TestCase):
    ...
    def test_post_with_invalid_data(self):
        ...
        res = self.client.post('/account/login/', data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.context['form'].errors['email'], ['Incorrect email or password.'])
        data = {
            'email': self.VALID_EMAIL,
            'password': 'incorrect'
        }
