...
from task.views import get_redirect_url, get_data_to_search, get_field_in_search_dict, check_field_in_search_dict
...
class TestTaskCreateView(BaseLoginRequiredTestCase):
    ...
    def test_post(self):
        ...
        self.assertEqual(res.url, reverse(get_redirect_url(Task.objects.first().duration)))
    ...
    def test_utils_function(self):
        self.assertEqual(get_data_to_search({'type': 'anything','extra_data': 'anything'}), {'type': 'anything'})
        self.assertEqual(get_data_to_search({'type': 'anything','duration': 'anything'}), {'type': 'anything', 'duration': 'anything'}) # noqa
        self.assertEqual(get_data_to_search({'type': 'anything','duration': 'anything'}, ['duration']), {'type': 'anything'}) # noqa
        self.assertTrue(check_field_in_search_dict('duration', {'duration__gt': 'anything'}))
        self.assertFalse(check_field_in_search_dict('duration', {'type': 'anything'}))
        self.assertEqual(get_field_in_search_dict('duration', {'duration__gt': 'anything'}), 'anything')
        self.assertEqual(get_field_in_search_dict('duration', {'type': 'anything'}), None)
...
class TaskScoreView(BaseLoginRequiredTestCase):
    ...
    def test_get(self):
        ...
        qs = self.user.tasks.filter(is_completed=True, duration=Task.TASK_DUERATION_DAILY).values('created_at__date').annotate(
...
class TestTaskListView(BaseLoginRequiredTestCase):
    ...
    def test_get(self):
        ...
        qs = self.user.tasks.filter(duration=Task.TASK_DUERATION_LONG_SHORT_TERM).order_by(
                                    '-id').values("id", "name", "deadline", "type", "is_completed")
...
class TestTaskDailyView(BaseLoginRequiredTestCase):
    ...
    def test_get(self):
        ...
        tasks = self.user.tasks.filter(duration=Task.TASK_DUERATION_DAILY, created_at__date=timezone.now().date()).distinct().order_by('-id') # noqa
        ...
        self.assertEqual(list(context['tasks']), list(tasks.order_by('deadline').values("id", "name", "deadline", 'is_completed', "type"))) # noqa
...
class TestTaskDetailView(BaseLoginRequiredTestCase):
    ...
    def test_post(self):
        ...
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.json(), {'message': 'Successfully Marked!', 'is_completed': not is_completed})
